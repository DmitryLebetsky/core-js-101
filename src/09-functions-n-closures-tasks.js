...
function getPolynom(...args) {
  ...
  return (x) => args.reduce((acc, currentValue, index, arr) => acc
  + currentValue * (x ** (arr.length - index - 1)), 0);
}
...
function logger(func, logFunc) {
  return (...args) => {
    const logMassage = args.reduce((acc, currentValue) => `${acc}${acc === '' ? '' : ','}${JSON.stringify(currentValue)}`, '');
    logFunc(`${func.name}(${logMassage}) starts`);
    const result = func(...args);
    logFunc(`${func.name}(${logMassage}) ends`);
    return result;
  };
}
...
function getIdGeneratorFunction(startFrom) {
  ...
  return () => {
    const shouldReturn = currentValue;
    currentValue += 1;
    return shouldReturn;
  };
}
