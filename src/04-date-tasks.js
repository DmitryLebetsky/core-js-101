...
function timeSpanToString(startDate, endDate) {
  ...
  const milliseconds = timeBetween % 1000;
  const seconds = Math.floor((timeBetween / 1000) % 60);
  const minutes = Math.floor((timeBetween / 60000) % 60);
  const hours = Math.floor((timeBetween / 3600000) % 24);
...
function angleBetweenClockHands(date) {
  ...
  let angleInDegrees = Math.abs((0.5 * (60 * hours + minutes)) - (6 * minutes));
